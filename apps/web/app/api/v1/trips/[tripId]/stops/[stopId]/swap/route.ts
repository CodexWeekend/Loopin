...
import { swapTripStop } from '@/lib/local-db';
import {
  handleRouteError,
  jsonOk,
  parseJsonBody,
...
export async function POST(request: Request, context: StopRouteContext) {
  ...
  try {
    return jsonOk(swapTripStop(session.userId, params.data.tripId, params.data.stopId, body.data));
  } catch (error) {
    return handleRouteError(error, 'swap trip stop');
  }
}
